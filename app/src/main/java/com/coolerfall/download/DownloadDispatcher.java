...
public class DownloadDispatcher extends Thread {
    ...
    /**
     * Sleep TIME_IN_MILLISECOND before download.
     */
    private static final int SLEEP_BEFORE_DOWNLOAD = 1500;

    /**
     * Sleep TIME_IN_MILLISECOND before restrying download.
     */
    private static final int SLEEP_BEFORE_RETRY = 3500;

    /**
     * Http connection TIME_IN_MILLISECOND out.
     */
    private static final int DEFAUL_TIME_OUT = 60 * 1000;
    ...
    /**
     * Redirection TIME_IN_MILLISECOND happens in this request.
     */
    ...
    public void run() {
        ...
        while (true) {
            try {
				...
				/* we may have been interrupted because it was TIME_IN_MILLISECOND to quit */
                if (mQuit) {
                    if (request != null) {
                        request.finish();
                    }
    ...
    private void updateProgress(DownloadRequest request, long bytesWritten, long totalBytes) {
        ...
        if (bytesWritten != totalBytes && currentTimestamp - mLastProgressTimestamp < request
                .getProgressInterval()) {
            return;
        }
    ...
    private void updateFailure(DownloadRequest request, int statusCode, String errMsg) {
        ...
        if ((statusCode == HTTP_INVALID || statusCode == HTTP_ERROR_SIZE) && request
                .getRetryTime() >= 0) {
            try {
...
//            Log.d("MMM", "download statusCode : " + statusCode);
...
//            Log.d("MMM", "download finished");
    ...
    private void transferData(HttpURLConnection conn, DownloadRequest request) throws IOException {
        ...
        InputStream is = null;
        File file = new File(request.getDestFilePath());
//        Log.i(TAG, "download file path : " + file.getAbsolutePath());
            ...
            if (is != null) {
                ...
                while (true) {
                    ...
                    if (Thread.currentThread().isInterrupted() || request.isCanceled()) {
                        Log.i(TAG,
                                "download has canceled, download id: " + request.getDownloadId());
                        request.finish();
                        return;
                    }
...
//                    }
                    ...
                    //					Log.i(TAG, "download length : " + length);

                    if (length == -1) {
                        long fileSize = new File(request.getDestFilePath()).length();

						/* deliver progress callback before deliver success */
                        updateProgress(request, fileSize, contentLength);

                        //						Log.i(TAG, "fileSize : " + fileSize + " contentLength : " + contentLength);
    ...
    private long getContentLength(HttpURLConnection conn) {
        ...
        //		Log.w(TAG, "transferEncoding " + transferEncoding);
